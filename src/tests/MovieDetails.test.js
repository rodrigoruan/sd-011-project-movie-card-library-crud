...
describe('4 - Faça uma requisição para buscar o filme que deverá ser renderizado dentro de `Movie Details`', () => {
  ...
  xit('Será validado se `MovieDetails` exibe o texto "Carregando..." enquanto estiver fazendo a requisição', async () => {
  ...
  xit('Será validado se `MovieDetails` exibe o título, o subtítulo, a sinopse, a imagem e o gênero do filme', async () => {
    for (const movie of readMovies()) {
  ...
  xit('Será validado se `MovieDetails` contém um botão com o texto "VOLTAR" que redireciona para a página inicial', async () => {
    for (const movie of readMovies()) {
  ...
  xit('Será validado se `MovieDetails` contém um botão com o texto "EDITAR" que redireciona para a página de edição de filme', async () => {
    for (const movie of readMovies()) {
      const { unmount, findByText } = renderPath('/movies/' + movie.id);
      await waitFor(() => movieAPI.getMovie(movie.id));
...
describe('7 - Adicione um link para deletar um cartão em `MovieDetails`', () => {
  ...
  xit('Será validado se `MovieDetails` contém um link com o texto "DELETAR"', async () => {
    for (const movie of readMovies()) {
      const { unmount, findByText } = renderPath(`/movies/${movie.id}`);
      await waitFor(() => movieAPI.getMovie(movie.id));
  ...
  xit('Será validado se o link "DELETAR" faz uma requisição para a API para excluir o filme em questão', async () => {
