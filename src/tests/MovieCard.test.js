...
describe('3 - Insira um link para a página de detalhes de um filme dentro de `MovieCard`', () => {
  ...
  xtest('Será validado se cada `MovieCard` exibe pelo menos o título e a sinopse de seu respectivo filme', async () => {
  ...
  xtest('Será validado se cada `MovieCard` contém um link com o texto `VER DETALHES` que redireciona para a página de detalhes do filme', async () => {
    const { unmount, getAllByText } = renderPath('/');
    await waitFor(() => movieAPI.getMovies());
